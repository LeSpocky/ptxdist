...
class CPECheckGenerator(SbomGenerator):
    ...
    def _load_dictionary(self, env):
        ...
        if filename and not path.exists(filename):
            raise ReportException(f'CPE dictionary file "{filename}" is missing!')

        if (
            cache_filename
            and path.exists(cache_filename)
            and (
                not filename or path.getmtime(cache_filename) > path.getmtime(filename)
            )
        ):
            try:
    ...
    def check_package(self, result, pkg):
        ...
        for vendor, product in self.get_cve_products(pkg):
            if product not in self.cpe_db:
                ...
                continue
            cpe = CPE2_3(f"cpe:2.3:*:{vendor}:{product}:{version}:{update}:*:*:*:*:*:*")
            ...
            for v in vendors:
                ...
                if found and with_version:
                    ...
                    raise ReportException(f"cpe matching logic error for {pkg_name}")
    ...
    def build(self, data):
        ...
        while self.pkgs:
            ...
            if self.current_pkg in data["images"]:
                self.pkgs.update(set(data["images"][self.current_pkg].get("pkgs", ())))
            else:
                self.check_package(result, data["packages"][self.current_pkg])
