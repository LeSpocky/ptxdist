...
class LatexGenerator(Generator):
    ...
    def format_dot(self, args):
        ...
        return (
            ...
            dot2tex(
                dot,
                docpreamble="\\usepackage[xetex]{hyperref}",
                figonly=True,
                format="pgf",
                autosize=True,
            ),
    ...
    def init_dot(self, pkgs, limit):
        ...
        with Pool() as pool:
            dots = pool.map(
                self.format_dot, [(packages, pkg) for pkg in packages.values()]
            )
        self.__dot = {}
        for pkg, dot in dots:
            self.__dot[pkg] = dot
    ...
    def finalize(self, document, output):
        ...
        while True:
            ret = subprocess.run(
                ["xelatex", "-halt-on-error", path.basename(tmp)],
                env=env,
                capture_output=not self.verbose,
                text=True,
                cwd=path.realpath(output_directory),
            )
            if ret.returncode != 0:
                if not self.verbose:
            ...
            new_aux_hash = hashlib.sha256(open(base + ".aux", "rb").read()).hexdigest()
