...
class Generator:
    ...
    def create_dot_recurse(self, pkgs, pkg, level, hit_deps):
        ...
        licenses = " ".join(i + "\\\\" * (w % 3 == 2) for w, i in enumerate(licenses))
...
""" % (pkg["name"], pkg["name"], self.escape(pkg["name"]), licenses)
        ...
        for dep in pkg["builddeps"]:
            ...
            data += """"%s" -> "%s"[dir=back];
""" % (pkg["name"], dep)
            data += self.create_dot_recurse(pkgs, pkgs[dep], level + 1, hit_deps)
        return data
...
"""
    ...
    def get_cve_version(self, pkg):
        ...
        tmp = version.split(":")
        if len(tmp) > 2:
            raise ReportException(f'cve-version "{version}" contains more than one ":"')
