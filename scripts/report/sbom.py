    ...
    def generate_purls(self, pkg):
        ...
        for url in pkg.get("url", []):
            if m := self.PYPI_RELEASE.match(url):
                ...
                purls.append(f"pkg:github/{m.group(1)}/{m.group(2)}@{m.group(4)}")

            elif m := self.GITHUB_RELEASE.match(url):
                purls.append(f"pkg:github/{m.group(1)}/{m.group(2)}@{m.group(3)}")
                ...
                if (
                    ...
                    or url.startswith("https://crates.io")
                    or url.startswith("https://static.crates.io")
                ):
        ...
        with open(series) as f:
            for line in f:
                ...
                if not path.exists(patch):
                    raise ReportException(f"Patch missing for {pkg['name']}: {patch}")
