...
class SpdxSbomGenerator(SbomGenerator):
    ...
    def add_packages(self, pkgs):
        for pkg in sorted(pkgs):
            if (
                pkg not in self.spdx_pkgs
                and pkg != self.current_pkg
                and pkg not in self.pkgs
            ):
    ...
    def build_package(self, document, pkg):
        ...
        for source in pkg.get("source-packages", []):
            ...
            if spdx_source:
                document.add_relationship(spdx_source, "BUILD_DEPENDENCY_OF", spdx_pkg)
    ...
    def add_dependencies(self, document, pkg, spdx_pkg):
        for dep in pkg.get("builddeps", []):
            ...
            if dep in pkg.get("rundeps", []):
                ...
                document.add_relationship(spdx_dep, "BUILD_DEPENDENCY_OF", spdx_pkg)
        for dep in pkg.get("rundeps", []):
            if dep not in self.spdx_pkgs:
                raise ReportException(f'Missing packge "{dep}"')
            spdx_dep = self.spdx_pkgs[dep]
            if dep not in pkg.get("builddeps", []):
                document.add_relationship(spdx_dep, "RUNTIME_DEPENDENCY_OF", spdx_pkg)
    ...
    def add_external_refs(self, document, pkg, spdx_pkg):
        ...
        if pkg_doc:
            ...
            document.add_relationship(ref.externalDocumentId, "AMENDS", spdx_pkg)
    ...
    def build(self, data):
        ...
        creationInfo.creators = [
            ...
            f"Tool: ptxdist-{data['ptxdist']['version']}",
        ]
        ...
        document.documentNamespace = (
            f"http://spdx.org/spdxdoc/{document.name}/{namespace_uuid}"
        )
        ...
        while self.pkgs:
            ...
            if self.current_pkg in data["images"]:
                self.add_packages(set(data["images"][self.current_pkg].get("pkgs", ())))
                self.build_package(document, data["images"][self.current_pkg])
            else:
                self.build_package(document, data["packages"][self.current_pkg])
        ...
        if target:
            document.add_relationship(document, "DESCRIBES", self.spdx_pkgs[target])
